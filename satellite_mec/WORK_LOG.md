...
### M15 结果（2026-06-21，跑到 3/5 用户叫停）⚠️ 重大发现
跑了 V=5/25/50 @32K（各单次训练），**用户叫停**，原因是出现比"V 选谁"更重要的问题：

| V | CR(32K单训) | HL/slot | 对照 |
|---|---|---|---|
| 5 | 0.769 | 2.58e-4 | |
| 25 | 0.750 | 2.67e-4 | |
| 50 | **0.745** | **2.89e-4** | vs 已部署 V=50 ckpt **0.786 / 1.74e-4** |

**核心问题：训练 seed 方差 >> V 效应。** 同样 V=50/32K/定稿超参，重训得 CR 0.745，而已部署 checkpoint 是 0.786——**差 4.1pp CR / +66% HL**。V 点之间才差 1–2pp，**完全被训练噪声淹没** → 单次训练**排不出 V 的序**，且 32K 趋势(V=5>25>50 递减)不支持"V=50 最优"。
**更深含义**：headline 的 LyaMAPPO 0.786 是**单次幸运训练**；这是比 eval-seed(已做 n=10) 更严重的**训练-seed 可复现性**问题。
**未决**：如何处理（多训练 seed 求均值 / 改"V 在噪声内鲁棒"叙事 / 复核主 checkpoint 代表性）——待与用户定。结果数据留痕于 `docs/sensitivity_v_32k.json`(3点)。
