...
本节将 LyaMAPPO 与四个 baseline——学习型调度器 TD3-Sched\cite{huang_tmc_2024}、最强经典启发式 MHSPO\cite{mhspo_ref}、博弈论卸载 GDCO\cite{chen_tmc_2025}、平凡下界 LSO——在同一 Lyapunov-DVFS 物理基底上对比，按 outline 报告五个指标：寿命损耗、系统时延、用户满意度、系统能耗、任务积压。所有标量为 n=10 独立种子的 mean ± std（CI 极紧，std ≤ 0.004），系统级量按 §1.4 投影至 N=192。

### 2.0 总体结果与 Pareto 定位

表 1 给出五个算法在 N=192 口径下的全部指标。

**表 1　对比实验总体结果（N=192，mean ± std，n=10 独立种子）**
...
> 加粗为该列竞争性最优。系统能耗 LSO（4852）数值最低，但其满意度仅 0.269、近乎瘫痪，不构成竞争（见 §2.4 LSO 反例）；故能耗列以 LyaMAPPO 为强策略最优。

**Pareto 定位（诚实框架）**。表 1 中**没有任一算法支配 LyaMAPPO**：TD3-Sched 在满意度、时延、队列三项上领先，但其系统累计 HL（463）与总能耗（12217 kJ）约为 LyaMAPPO 的 **2.5 倍**；LyaMAPPO 以约 1–5pp 的满意度让步，换得在**不可逆的电池健康维度**上的断层领先。图 `pareto_sat_hl.png` 在「满意度–累计 HL」平面上画出这一权衡前沿：LyaMAPPO 位于低 HL 端的拐点，TD3/MHSPO 位于高满意度但高 HL 端。图 `bars_4metrics.png` 给出四指标的归一化柱状总览。
...
**能耗再分解**。计算能耗 \(E \propto \overline{f^3} = \bar f^{\,3}\times D\)，其中 \(D = \mathrm{mean}(f^3)/\mathrm{mean}(f)^3\) 为离散/峰值乘子（`dispersion_D`）。相对 LyaMAPPO：

**表 3　能耗双因子分解（相对 LyaMAPPO）**
...
→ LyaMAPPO 的省能**主要来自跑更低的时钟**（level ~1.4–1.5×，对应 §2.2 的 ~0.27 s 时延与 ~5pp 吞吐让步），**负载均衡削频率峰为次要因素**（dispersion ~1.1–1.35×）。此处诚实指出：削峰并非主因。
...
**机理**。Lyapunov drift 最小化保证队列的均值率稳定性（mean-rate stability）：各策略队列曲线在 5400 时隙内平稳、不发散。LyaMAPPO 与 MHSPO 同享 Lyapunov 队列控制故积压相当；GDCO/LSO 因卸载能力弱（GDCO 博弈均衡偏保守、LSO 零卸载）而积压显著更高。LyaMAPPO 相对 TD3 略高的积压（+63 tasks）与其更低时钟的能耗策略一致——属可控代价，未突破稳定性。

**小结（对比实验）**。综合五指标：TD3-Sched 在吞吐/时延/队列上领先但电池维度（HL+能耗）约 2.5×；LyaMAPPO 以 ≤5pp 吞吐、~0.27 s 时延、+63 队列的可控让步，换得累计 HL ≈ 1/3、能耗 ≈ 一半。在不可逆的电池健康维度上，LyaMAPPO 于竞争性策略中断层第一，且 CI 极紧（std ≤ 0.004），排名稳健——这是一个清晰的 Pareto 权衡，而非无限定的全面最优。

## 3 消融实验（Ablation）

为厘清 LyaMAPPO 中各组件的贡献，本节将两个关键组件分别剥离：**电池感知**（得到 MAPPO-NoDoD）与**强化学习**（得到 LyapunovGreedy），在同一物理基底、同一 n=10 评估口径下对比。按 outline 报告四个指标：寿命损耗、系统时延、用户满意度、系统能耗。

> **诚实披露**：MAPPO-NoDoD 是**合并消融**——`--no_battery` 同时移除了 Lyapunov 动作代价中的电池/DoD 项 与 outcome 奖励中的 \(W_{\text{HL}}\)（置 0），因此它界定的是"电池感知整体"的作用，而非单一项。

### 3.0 总体结果

**表 4　消融实验结果（N=192，mean ± std，n=10 独立种子）**
...
**LyapunovGreedy：能耗 3356 kJ 最低**——同样是"摆烂"假象：低时钟 + 少完成任务自然少耗能，但满意度仅 0.425，不构成竞争（与 §2.4 的 LSO 反例同理：低能耗 ≠ 好策略）。

### 3.5 小结（消融实验）

两个消融印证了 LyaMAPPO 两个核心组件**各自不可或缺**：
