...
- **论文指标口径（用户拍板 2026-06-20）**：**论文主指标=满意度（satisfied/(done+timeout)）**，
  **CR（done/arrived）仅保留在内部实验**、不进论文台面。依据：实测各策略 CR≈满意度（差≤0.3pp），
  满意度诚实反映吞吐、无丢任务注水。正文留一句"丢弃率极低、满意度即服务吞吐"堵审稿漏洞。
